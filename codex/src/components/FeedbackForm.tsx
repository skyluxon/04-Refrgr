...
export function FeedbackForm({ page }: { page:"general"|"special" }) {
  ...
  return <section className="section bg-[#071d3b] text-white"><div className="container-site"><div className="max-w-2xl"><p className="eyebrow !text-blue-300">1분 피드백</p><h2 className="title">더 나은 제품 탐색을 위해 알려주세요</h2><p className="mt-3 text-blue-100">현재 응답은 MVP 검증용이며 저장되지 않습니다.</p></div>
...
function Scale({name,label}:{name:string;label:string}) { return <fieldset><legend className="font-bold">{label}</legend><div className="mt-3 flex gap-2">{[1,2,3,4,5].map(n=><label key={n} className="grid h-11 w-11 place-items-center rounded-lg border border-blue-300/40"><input className="sr-only peer" required type="radio" name={name} value={n}/><span className="peer-checked:font-black peer-checked:text-blue-300">{n}</span></label>)}</div><p className="mt-2 text-xs text-blue-200">1 전혀 그렇지 않다 · 5 매우 그렇다</p></fieldset> }
